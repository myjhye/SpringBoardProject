...
$(function(){
	$('#writeBtn').on('click',function(){ // 글쓰기 버튼을 눌렀을 때
		let name=$('#name').val(); // ID 값이 #name인 게 비어 있으면
		if(name.trim()=="")
		{
			$('#name').focus(); // #name 칸에 포커스
			return;
		}
		
		let subject=$('#subject').val(); // 나머지도 동일
		if(subject.trim()=="")
		{
			$('#subject').focus();
			return;
...
<body>
  <div class="container">
    <h1>글쓰기</h1>
    <div class="row">
      <form method=post action="insert_ok.do" id="frm"> <!-- 입력된 내용을 insert_ok.do에 담아 처리한다 -->
      <table class="table">
       <tr>
